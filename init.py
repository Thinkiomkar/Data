...
password = 'TTPL@123'
connection_string = f"DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};DATABASE={database};UID={username};PWD=
...
def main():
    ...
    cursor = conn.cursor()
    cursor.execute("EXEC DemoData 

@cmsyscode=?", (cmsyscode,))  
    result = cursor.fetchall() 
    columns = [column[0] for column in cursor.description]
    result_reshaped = [tuple(row) for row in result]
    df = pd.DataFrame(result_reshaped, 
...
T1.name = 'Leads Status'
    ...
    fig_html = py.plot(fig, 

output_type='div', include_plotlyjs='cdn', config={'displayModeBar': False})
    return fig_html 
