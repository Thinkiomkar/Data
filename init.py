...
def main():
    ...
    fig.update_layout(modebar={'remove': ['zoom2d', 'autoscale2d', 'pan2d', 'lasso2d', 'resetScale2d','logo','ModeBar'],},showlegend=False,)
...
def main3():
   ...
   'Lost Lead': 'rgb(255, 0, 0)',    
    ...
    fig.update_traces(textfont_size=12, textangle=0, textposition="outside", cliponaxis=False,  hovertemplate='%{x}<br><b>%{y}</b>') 
    fig.update_layout(xaxis=dict(title='<b>Sources<b>'),
                  yaxis=dict(title='<b>Count<b>'),
                legend=dict(title='<b>Lead Status<b>'),
                title=dict(text="<b>Lead Conversions From Sources<b>"),
                plot_bgcolor='white',
                 height=600, width=1350,
                 font_size=14)
    fig.update_layout(modebar={'remove': ['zoom2d', 'autoscale2d', 'pan2d', 'lasso2d', 'resetScale2d','logo','ModeBar']},showlegend=True,)
